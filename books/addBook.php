<?php
    ...
    if ($_POST) {
        ...
        if (empty($title)) {
            ...
            array_push($errors, 'The title must be at least 5 characters');
        } elseif (strlen($title) > 100) {
            // var_dump('the length must be less than 100');
            array_push($errors, 'The title must less than 100 characters');
        }

        if (empty($year)) {
            array_push($errors, 'The year is empty, please add a value.');
        } else if(strlen($year) > 4){
            array_push($errors, 'The year length myst be less than 4 characters.');
        }

        if (empty($author)) {
            // var_dump('the author is empty');
            array_push($errors, 'The author is empty, please add a value');
        } elseif (strlen($author) < 5) {
            // var_dump('the length must be at least 5');
            array_push($errors, 'The author must be at least 5 characters');
        } elseif (strlen($author) > 100) {
            // var_dump('the length must be less than 100');
            array_push($errors, 'The author must less than 100 characters');
        }

        if (empty($description)) {
            // var_dump('the description is empty');
            array_push($errors, 'The description is empty, please add a value');
        } elseif (strlen($description) < 10) {
            // var_dump('the length must be at least 10');
            array_push($errors, 'The description must be at least 10 characters');
        } elseif (strlen($description) > 65535) {
            var_dump('the length must be less than 65535');
            array_push($errors, 'The description must less than 65,535 characters');
        }
    }
